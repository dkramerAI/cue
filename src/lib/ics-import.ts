...
export function parseICSContent(icsText: string): ParsedICSResult {
  ...
  for (const line of lines) {
    ...
    // VALARM sub-blocks: only TRIGGER matters; ignore their other properties
    // so an alarm's DESCRIPTION/UID never overwrites the event's own fields.
      ...
      if (key === "TRIGGER") {
        ...
        if (minutes !== null) {
          currentEvent.reminders = [
            ...(currentEvent.reminders || []),
            { id: Math.random().toString(36).slice(2), minutes },
          ];
        }
      }
      continue;
